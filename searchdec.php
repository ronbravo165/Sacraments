...
<?php
	...
	if (isset($_POST['search'])) {
		...
		$query = mysqli_query($con,"SELECT * from deceased_tbl where fullname LIKE '%$searchq%'") or die("No record found.");

		$count = mysqli_num_rows($query);

		echo "<a href='deceased.php' style='float: right;'>BACK</a>";
		...
		if ($count == 0) {
			...
			while ($row = mysqli_fetch_array($query)) {
				...
				$output .='<tr><td><center>'.$bn.'</center></td><td><center>'.$pn.'</center></td><td>'.$ln.'</td><td><center>'.$fullname.'</center></td><td><a href="deceasededit.php?id='.$row['id'].'"><center>EDIT</center></a></td><td><a href="deceaseddelete.php?id='.$row['id'].'"><center>DELETE</center></a></td>';
			}
		}
	}
